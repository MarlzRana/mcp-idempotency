...
async def run_scenario(server_url: str, use_idempotency_key: bool) -> None:
    ...
    meta = {"idempotencyKey": "demo-payment-1"} if use_idempotency_key else None

    _pretty_print("Calling make_payment (first attempt, expect timeout)...", "⏱️", YELLOW)
    try:
        ...
        _pretty_print(f"First make_payment had a transport issue: ({error_type}).", "⏳", RED)
    ...
    async with Client(server_url) as client:
        final_balance = await client.call_tool("get_balance", {"account_uid": account_uid})
        final_txs = await client.call_tool("get_transactions", {"account_uid": account_uid})
